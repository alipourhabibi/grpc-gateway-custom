...
var (
	crudTemplates template.Template
)
...
`))
	...
	funcMap template.FuncMap = map[string]interface{}{
		...
		"OptionExtractor": func(i string) map[string]map[string]string {
			// [google.api.http]:{get:"/destroy"}  [profile.my_option]:"true"
			// return i
			n := strings.Fields(i)
			...
			for _, v := range n {
				...
				err := yaml.Unmarshal([]byte(re2.FindString(v)), tt)
				if err != nil {
					fmt.Println(err)
				}
				m[re.FindString(v)] = tt
			}
			return m
		},
	}
...
{{template "request-func-signature" .}} {
	...
	*requ.Request.Query = req.URL.Query().Encode()
	...
	// {{ OptionExtractor ( printf "%v" .Binding.Method.Options) }}
