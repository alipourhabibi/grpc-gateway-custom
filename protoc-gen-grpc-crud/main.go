// Command protoc-gen-grpc-crud is a plugin for Google protocol buffer
...
// into your $PATH with a name "protoc-gen-grpc-crud" and run
//
//	protoc --grpc-crud_out=output_directory path/to/input.proto
//
...
import (
	...
	"github.com/grpc-ecosystem/grpc-gateway/v2/protoc-gen-grpc-crud/internal/gencrud"
	"github.com/grpc-ecosystem/grpc-gateway/v2/protoc-gen-grpc-crud/internal/gencrud/configs"
...
func init() {
	grpclog.NewLoggerV2WithVerbosity(os.Stdout, os.Stdout, os.Stdout, 0)
}
...
func main() {
	...
	err = yaml.Unmarshal(f, &configs.Confs)
	if err != nil {
		panic(err)
	}
	...
	protogen.Options{
		...
		generator := gencrud.New(reg, *useRequestContext, *registerFuncSuffix, *allowPatchFeature, *standalone)
